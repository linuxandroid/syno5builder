...
(function(){
	...
	/* SYNO: check Safari 5.0.x  */
	isSafari5_0 = isSafari && check(/version\/5.0/),
        isIE = !isOpera && check(/msie/),
		/* SYNO: use trident to check IE8/9  */
		isIE9 = isIE && check(/trident\/5/),
        isIE8 = isIE && !isIE9 && check(/trident/),
        isIE7 = isIE && !isIE8 && !isIE9 && check(/msie 7/),
	...
	if (isIE9 && 8 === document.documentMode) {
		isIE8 = true;
	}
...
/* SYNO: override setInterval/clearInterval for IE to prevent Java bug */
(function() {
	if (!Ext.isIE) {
		return;
	}
	...
	window.setInterval = function(fn, mill) {
		var tId = ++id,
			runner = function() {
				fn();
				if (task[tId]) {
					task[tId] = setTimeout(arguments.callee, mill);
				}
			};

		task[tId] = setTimeout(runner, mill);
		return tId;
	};
...
/* SYNO: Bug#13077, fix the toLocaleString function for some browsers */
if (Ext.isChrome ||
        (Ext.isMac && Ext.isGecko) ||
        (Ext.isWindows && Ext.isSafari)) {
...
Ext.TaskMgr = new Ext.util.TaskRunner();(function(){
	...
	Ext.lib.Dom = {
        ...
        getXY : function(el) {
			...
			/* SYNO: fix for DSM Bug#16819 */
			try {
				b = el.getBoundingClientRect();
			} catch(err) {
				b = el.lastBoundingClientRect;
			}
			el.lastBoundingClientRect = b;
...
Ext.lib.Ajax = function() {
    ...
    function handleReadyState(o, callback, opts){
    callback = callback || {};
        var conn = o.conn,
            tId = o.tId,
            poll = pub.poll,
	...
	/* SYNO: support for HTML5 Upload */
	if (opts && opts.html5upload && opts.progress && o.conn.upload) {
		if (!callback.scope) {
			o.conn.upload.onprogress = opts.progress.createDelegate(this, [], true);
		} else {
			o.conn.upload.onprogress = opts.progress.createDelegate(callback.scope, [], true);
		}
        }
    }
    ...
    /* SYNO: support for HTML5 Upload */
    function html5uploadRequest(method, uri, callback, postData, opts) {
        var o = getConnectionObject() || null;
        ...
        if (o) {
            ...
            handleReadyState(o, callback, opts);
            if (!opts.html5upload || (opts.html5upload && !opts.chunkmode) || !Ext.isFunction(o.conn.sendAsBinary)) {
                o.conn.send(postData || null);
            } else {
                o.conn.sendAsBinary(postData || null);
            }
        }
        return o;
    }
	...
	/* SYNO: support for HTML5 Upload */
	html5upload: function(method, uri, cb, data, options) {
            if(options){
                var me = this,
            ...
            return html5uploadRequest(method || options.method || "POST", uri, cb, data, options);
	},
